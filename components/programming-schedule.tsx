...
import { Calendar, Clock, Play, Pause, Volume2 } from "lucide-react"
...
export default function ProgrammingSchedule() {
  ...
  const handlePlayJingle = (programId: string, jingleUrl: string) => {
    if (playingJingle === programId) {
      ...
      // Reproducir nuevo jingle
      const audio = new Audio(jingleUrl)
      audioRef.current = audio
      setPlayingJingle(programId)
      ...
      audio.onerror = () => {
        setPlayingJingle(null)
        alert("Error al reproducir el jingle")
      }
    }
  }
        ...
        <CardContent className="p-6">
            ...
            {schedule[activeDay]?.length > 0 ? (
              schedule[activeDay].map((program) => (
                <div
                  ...
                  onClick={() => handlePlayJingle(program.id, program.jingleUrl)}
                >
                  <div className="flex items-center gap-4">
                    <div className="bg-red-100 p-2 rounded-full group-hover:bg-red-200 transition-colors">
                      {playingJingle === program.id ? (
                        <Volume2 className="h-5 w-5 text-red-600 animate-pulse" />
                      ) : (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <Button
                      ...
                      onClick={(e) => {
                        e.stopPropagation()
                        handlePlayJingle(program.id, program.jingleUrl)
                      }}
                    >
                      {playingJingle === program.id ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />}
                    </Button>
                  </div>
                </div>
              ))
          ...
          <div className="mt-6 pt-4 border-t border-gray-200">
            <p className="text-sm text-gray-600 flex items-center gap-2">
              ...
              Haz clic en cualquier programa para escuchar su jingle
            </p>
